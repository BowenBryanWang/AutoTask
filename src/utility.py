...
def Task_UI_grounding_prompt(task, current_path_str, similar_tasks, similar_traces, semantic_info_list, next_comp, Knowledge, UI_long_term_knowledge, hint):
    content = {}
    content["Task"] = task
...
def Knowledge_prompt(TASK, ACTION_TRACE, log):
    return [
        {
            "role": "system",
...
Among them, another AI agent is an intelligent body for UI task execution, which has a workflow of predict-evaluate-decide, i.e., it first calculates the follow-up of each page according to the UI jump library, then scores the elements on the page related to the user's task, and finally decides the status of task completion. Eventually the task is completed successfully.
...
1, Reproduce the ACTION_TRACE and log file step by step, focusing on each page and analyzing if each step has expectedly leading to the right outcome. Reason step by step on the insights of the relationship between TASK and ACTION-TRACE from a higher level and generate SOFT knowledge of connecting them, it could be hints, experiences, or more.
    1.1, Specificly, think step by step on what kind of SOFT knowledge can be extracted from them to guide better action-selection, which means how an AI agent choose correct next-step when exploring in the UI.(i.e. "In settings page, network functions would be more related to 'Network' button"......)
...
Here are some other notes and requirements:
a, Do not generate knowledge that is too shallow and low-level, e.g., Use scrolling to find more content, as this is already a natural implementation of the system we are designing, and generating knowledge that is so simple and shallow will pollute the knowledge base;
    a.1, low-level knowledge examples: tasks are marked as complete when there is specific content on the page; clicking back when the user navigates to an unrelated page; about device related tasks in the settings are in the device sub-option; and so on.
        This kind of knowledge is too shallow, and naturally comes with any assistant intelligence that operates a mobile phone; we need more inspiring, deeper examples!
b, Do not generate knowledge that is too abstract and short, the knowledge generated will be used to assist in the subsequent task execution process, so consider being moderately specific.
c, Generating more generalisable knowledge, i.e. task-independent knowledge, decouples the higher level knowledge of the user's task for subsequent use by the AI agent.
"""
        }, {
            "role": "user",
            "content": """TASK:{}
ACTION TRACE:{}
log file:{}""".format(TASK, ACTION_TRACE, log)
        },
        {
            "role": "user",
            "content": """'''HTML
