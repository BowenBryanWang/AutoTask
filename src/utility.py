...
3,go on: After the last action and based on the newest UI screen, the operation sequence is on the correct track, the current UI is related to the UI task and we can continue to perform further operations to complete the task, but the task has not been completed. There is at least one UI element that is related with the UI task and interacting with it helps carrying out the task. Further Actions should be taken on the current (may also be the subsequent pages) UI page.
